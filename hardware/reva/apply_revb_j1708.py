...
# Receiver stays enabled (/RE=0) for bus/echo/collision monitoring.
# MCU drives DI (J1708_TX) and DE (J1708_DE); DE has a hardware pull-down
# so the TruckBox powers up silent on the multi-master J1708 bus.
...
for old, new in (
    ('CAN2_RX', 'J1708_RX'),
    ('CAN2_TX_SAFE', 'J1708_TX'),
    ('CAN2_H', 'J1708_B'),
    ('CAN2_L', 'J1708_A'),
):
    s = s.replace(old, new)

# J1708_DE is new in Rev.B; reuse the former CAN2 TX net for J1708_TX.
...
def assign_pad_net(block, padnum, name):
    """Set or add a net on a pad, including previously unused ESP32 pads."""
    m=re.search(r'\(pad\s+"?'+re.escape(str(padnum))+r'"?\s+',block)
    if not m:
        raise RuntimeError(f'pad {padnum} not found')
    j=balanced_block(block,m.start())
...
for pin, net in {
    1:'J1708_RX',   # RO
    2:'GND',        # /RE permanently enabled (low)
    3:'J1708_DE',   # DE from MCU, default low
    4:'J1708_TX',   # DI from MCU UART TX
    5:'GND',
    6:'J1708_A',
    7:'J1708_B',
    8:'3V3_MAIN',
...
# Reserve adjacent free ESP32-C6 module pins for J1708 transmit control:
# module pad 17 = GPIO19 -> UART TX, pad 19 = GPIO21 -> DE.
...
for a,b,blk in iter_blocks(s,'segment'):
    ...
    if name=='3V3_MAIN' and same_pair(p1,p2,(69.3,49.135),(68.0,48.9)):
        drop=True
    # Old U4 pin-1 TX-safe stub is no longer the transmit input in SP3485 mode.
    if name=='J1708_TX' and (
        same_pair(p1,p2,(67.5,49.0),(69.3,46.595)) or
        same_pair(p1,p2,(68.5,49.0),(69.3,46.595))
    ):
        drop=True
...
items=[
    # Receive path (RO -> existing ESP32 GPIO20 path).
    seg('J1708_RX',69.300,46.595,65.500,46.500,.22),

    # Keep CAN1 mode routing intact; U4 pin8 is now VCC.
    seg('CAN_MODE',78.000,40.595,78.000,44.405,.28,'B.Cu'),
    seg('3V3_MAIN',74.700,46.595,76.000,45.700,.30),
    via('3V3_MAIN',76.000,45.700),

    # DE local link and fail-safe pull-down at R4.
    seg('J1708_DE',68.500,49.000,69.300,49.135,.22),
    seg('GND',67.500,49.000,66.700,49.000,.25),
    via('GND',66.700,49.000),

    # GPIO19 / module pad17 -> SP3485 DI (pin4), routed on B.Cu.
    seg('J1708_TX',26.750,21.710,28.400,21.710,.22),
    via('J1708_TX',28.400,21.710),
    seg('J1708_TX',28.400,21.710,31.000,42.000,.22,'B.Cu'),
    seg('J1708_TX',31.000,42.000,61.500,47.000,.22,'B.Cu'),
    seg('J1708_TX',61.500,47.000,67.000,50.405,.22,'B.Cu'),
    via('J1708_TX',67.000,50.405),
    seg('J1708_TX',67.000,50.405,69.300,50.405,.22),

    # GPIO21 / module pad19 -> SP3485 DE (pin3), also on B.Cu.
    seg('J1708_DE',26.750,19.170,29.400,19.170,.22),
    via('J1708_DE',29.400,19.170),
    seg('J1708_DE',29.400,19.170,33.000,40.500,.22,'B.Cu'),
    seg('J1708_DE',33.000,40.500,60.500,45.500,.22,'B.Cu'),
    seg('J1708_DE',60.500,45.500,66.000,49.135,.22,'B.Cu'),
    via('J1708_DE',66.000,49.135),
    seg('J1708_DE',66.000,49.135,68.500,49.000,.22),
]
close=s.rfind(')')
if close<0:
...
if any(name in s for name in ('CAN2_H','CAN2_L','CAN2_RX','CAN2_TX_SAFE')):
